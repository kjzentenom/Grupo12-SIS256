...
WHERE
    c.id_destinatario = $miscorreos
    AND (c.estado = 'enviado' OR c.estado = 'abierto')
    AND c.tipo = 1
...
WHERE
    ...
    AND (c.estado = 'enviado' OR c.estado = 'abierto') 
    AND (c.tipo = 1)
    AND ((u_remit.correo LIKE '%$buscar%') OR (c.asunto LIKE '%$buscar%') OR (c.estado LIKE '%$buscar%'))
    ORDER BY $orden $asc 
    LIMIT $inicio,5;";
...
$arreglobec = [
    ...
    "pagina" => $pagina,
    "orden" => $orden,
    "objetivo" => "u_remit.correo",
    "nropaginas" => $nropaginas,
    "urlraiz" => "becorreo.php",
];

echo json_encode($arreglobec);
?>